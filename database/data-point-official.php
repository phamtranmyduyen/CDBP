...
while ($row = mysqli_fetch_array($result)) {
    ...
    $num++;
    if ($num == 9) {
        $sqlMaBCD = "SELECT bd.`Mã bảng chấm điểm`
        ...
        $queryMaBCD = DataProvider::executeQuery($sqlMaBCD);
        $rowMaBCD = mysqli_fetch_array($queryMaBCD);
        $sqlDiemBTV = "SELECT bdtc.`Điểm BCH chấm`
            FROM bangchamdiem_tieuchi bdtc
            WHERE bdtc.`Mã bảng chấm điểm` = '" . $rowMaBCD['Mã bảng chấm điểm'] . "'
        ...
        while ($rowDiemBTV = mysqli_fetch_array($queryDiemBTV)) {
            ...
            <input type='hidden' class='mabangchamdiem' value='".$rowMaBCD['Mã bảng chấm điểm']."'/>
            ...
            <th></th>
            <th>" . $rowDiemBTV['Điểm BCH chấm'] . "</th>
        </tr>";
