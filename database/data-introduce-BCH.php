...
echo "<div style='display:none' id='countintroduce-BCH'>$countData</div>";
// ----------------------------------------------------------
$sql = "SELECT DISTINCT gt.`Mã giới thiệu`, gt.`Nội dung giới thiệu`, gt.`Hình ảnh`, gt.`Hiển thị`
        FROM gioithieu gt, chitietgioithieu ctgt 
        WHERE gt.`Mã giới thiệu` = ctgt.`Mã giới thiệu`
        OR (gt.`Mã giới thiệu` NOT IN (
                                    SELECT ctgt.`Mã giới thiệu`
                                    FROM chitietgioithieu ctgt))
        ORDER BY gt.`Mã giới thiệu`";
...
while ($row = mysqli_fetch_array($result)) {
    ...
    if ($count > $start && $count <= $end) {
        ...
        "<tr>
            ...
            <th class='" . $row['Mã giới thiệu'] . "' style='color: red'>" . $row['Nội dung giới thiệu'] . "</th>
            <th style='color: red'>
                <input class='" . $row['Mã giới thiệu'] . "' type='hidden' value='" . $row['Hình ảnh'] . "'/>
                <img class='" . $row['Mã giới thiệu'] . "' style='width:50px; height: 50px' src='" . $row['Hình ảnh'] . "'/>
            </th>
            <th style='color: red'>
        ...
        <tr>
            ...
            <td style='border: none'><button style='border: none; background:none' class='edit-chitietgioithieu' value=" . $row1['Mã chi tiết giới thiệu'] . "><i class='fas fa-edit' ></button</i></td>
            <td style='border: none'><button style='border: none; background:none' class='del-chitietgioithieu' value=" . $row1['Mã chi tiết giới thiệu'] . "><i class='fas fa-trash-alt'></button></i></td>
        </tr>";
        }}
